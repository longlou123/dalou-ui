...
<script lang="ts">
  ...
  export default {
    ...
    setup(props, context) {
      ...
      onMounted( () => {
        html.value = Prism.highlight(props.component.htmlCode, Prism.languages.javascript, 'javascript')
        nextTick(() => {
          let { offsetHeight } = codePer._rawValue
          computedHeight.value = offsetHeight
        })
      })
      return {
        toggle,
    ...
    //   //由于打包后不支持
    //   //换成传入组件的形式
    //   this.html = Prism.highlight(this.component.htmlCode, Prism.languages.javascript, 'javascript')
    //   this.$nextTick(() => {
    //     const codePer = this.$refs.codePer
    //     let {
    //       height
    //     } = codePer.getBoundingClientRect()
